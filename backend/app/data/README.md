...
This directory holds **everything that runs outside the live API**: ingestion of raw text, label normalisation, quality validation, training-package assembly, baseline / fine-tune / ablation harnesses, and the pseudo-labelling pipeline. The live `/analyze` request path does **not** import from here — that's `services/`.

The files come in two layers:
...
- **Phase-named implementations** under the same names with `phase3_*` / `phase4_*` prefixes. These are still called by Makefile targets and contain the real logic; the capability-first names are thin wrappers. Phase 7.2 (`docs/REPO_TOUR.md §7`) will retire the phase-named layer.

The Makefile is the only authoritative entry point — `make data-prep`, `make train-smoke`, `make train-batch` all go through this directory.
...
| `label_normalization.py` | Map raw label strings to `{hawkish, dovish, neutral}`. Drop unmappable rows + log exceptions. Sample-weight by provenance. | `normalize_labels.py` |
...
### Embedding stores (used by the chunk-attention pooler + the chunk-aware teacher)
...
These produced one-shot artefacts already published under `data/artifacts/`; they are reachable via CLI but rarely re-run.
...
| `source_type_stratified_analysis.py` | Joiner from per-row predictions → per-source-type macro-F1 / accuracy / per-class P/R/F1. Run after each fine-tune batch. |

### Schema definitions

| File | What it does |
| --- | --- |
| `label_schemas.py` | Pydantic models for `MultiAxisLabeledRow` (stance / factor / certainty / topic axes). Sample-weight by provenance. |
| `source_type.py` | The 10-value `SOURCE_TYPE_VALUES` whitelist + `infer_source_type()` mapping from `(document_type, title)`. |

### Per-source scrapers

Live under `sources/`. See `sources/base.py` for the `BaseSourceScraper` protocol and the parallel `_VALID_SOURCE_TYPES` whitelist.

## See also

- `../../README.md` — quick start + tour pointer.
- `../../docs/REPO_TOUR.md` — single-page walkthrough of the whole codebase. §7 of that tour lists which files in this directory are alive vs historical.
