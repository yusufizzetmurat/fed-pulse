...
The evaluation surface (per-head `mae_bps` / `dir_acc` / `R²` with block-bootstrap CIs from `app.evaluation.regression_metrics`) does not change. Metrics still measure prediction against supervised target in bps, but the target is the FOMC-attributable component when the new mode is active.

## Why not the alternatives

Keeping the raw target and decomposing the prediction at inference time is identity-on-the-output: the head's loss surface and gradient flow stay tuned to the noisy raw move, and the model has no incentive to focus on the FOMC-attributable component during training. The point of #305 is to change the loss, not the reporting frame.
...
Signing the projection by `mp_surprise_path_factor` is plausible: path captures curve-shape surprise, level captures the immediate-policy surprise. Level wins here because Kuttner anchors there and because the 5-day forward horizon is dominated by the near-end of the curve where the level surprise has the bigger reaction. The path-anchored variant is a follow-up if the level variant ships.

## Methodology shift

The framing changes. Pre-#305: "the model predicts the post-FOMC 5-day rates move." Post-#305 with `--rates-target-mode=fomc_attributable`: "the model predicts the FOMC-attributable component of the 5-day rates move — the part of the move policy-watchers actually care about." The second framing matches the literature and is the one a Fed-watcher product surface should lead with.

R² numbers across the two modes are not directly comparable. The fomc-attributable target has a smaller raw variance than the unprojected move (because the projection collapses non-FOMC variance into a residual that gets discarded), so R² may be higher in the new mode without any actual lift in predictive power; the denominator changed. `mae_bps` and `dir_acc` stay comparable across modes (both in raw bps), but the interpretation shifts: `dir_acc` in fomc-attributable mode is "did the model correctly predict whether the move agreed with the surprise direction?", not "did the model correctly predict the move sign."
