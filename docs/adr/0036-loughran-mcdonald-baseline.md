...
The modal collapse (Strong_Modal + Weak_Modal → ``modal``) is a six-vs-seven dim choice. The seven-dim variant (strong and weak kept separate) is the alternative the literature sometimes ships; the six-dim union is picked here because the issue body specifies six features and because the strong / weak distinction does not change the headline diagnostic. A follow-up could sweep both and report whether the strong / weak split changes the head-to-head materially; the runner's arm-resolution is per-cell so the split would be a separate ``--arm`` value rather than a new family.
