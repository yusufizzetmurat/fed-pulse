...
The classifier sees a 3-way categorical, the regression head sees a continuous bps target. When both are trained jointly the gradient sharing on the shared backbone favours whichever loss has the cleaner signal — on the rates targets being trained, that is the regression branch. Running the aux head ON / OFF as an ablation is the cheap way to measure the auxiliary contribution.
