...
**Date:** 2026-06-02 · **Status:** COMPLETE — VALID but weak and asymmetric (a "hike detector"). See Result.
...
Verdict: the stance instrument is a valid concurrent measure of policy
hawkishness (primary significant, both decision-rule conditions met), but
asymmetric. It functions as a hike detector: hikes separate from cuts cleanly
(AUC 0.794), and the hike end now clears neutral (mean s +0.385). The dovish end
is inverted — cuts read more hawkish than holds (mean s −0.287 vs −0.655) — so
the instrument is not a fine-grained cut-vs-hold discriminator, and the
within-holds lead is weak (ρ +0.151).

Scope this sharpens for the descriptive layer: the dashboard's stance surface
can flag hiking-hawkish statements, but should not be read as a fine-grained
hawk–dove thermometer on the dovish side, nor imply "more dovish than the last
hold." Two concrete instrument-improvement leads follow. First, the dovish-end
resolution (cut vs hold) is where the head is blind; a targeted re-label or
training focus there would add the most. Second, the inverted cut/hold ordering
suggests that cut-associated statements carry hawkish framing the classifier
...
Text-as-prediction is closed in both directions (corners A–E, intraday,
late-fusion; reverse market→Fed null). What remains legitimate is
text-as-description: the multi-axis stance instrument `s = P(hawk) − P(dove)`
powers the dashboard's descriptive layer. That instrument has not been
validated against an objective external criterion. The clean-eval impasse is
that every stance label set is contaminated by training. This pre-registration
sidesteps that issue and validates `s` against the Fed's actual policy action,
which the classifier never saw. Construct and criterion validity, not
label-ranking.

## Anchor (objective, never in training)

Per scheduled FOMC meeting, the realised policy move `Δff_bps` is the step in
the FRED `DFEDTARU` funds-rate upper target around the meeting date (value 2
days after minus value 1 day before, ×100). The 2010–2026 distribution (n=130):
101 hold, 20 hike, 9 cut (−50→1, −25→8, 0→101, +25→14, +50→2, +75→4). Categories
...
`stance_daily.parquet` on the meeting date. The test relates the text's
hawkishness `s` to the concurrent (and next) policy action.
...
- **DIAGNOSTIC (descriptive, no decision weight):** within hold meetings only,
  ...
  hawkishness carrying forward guidance the current action does not?
...
strictly > 0. If both hold, strength is characterised by |ρ| (<0.2 negligible,
0.2–0.4 weak, 0.4–0.6 moderate, >0.6 strong) and the failure modes are read off
the diagnostics; together they bound what the descriptive layer can claim. If
the primary is not significant, the report records that the instrument does not
track realised policy (a stronger caveat the dashboard would then need). No
post-hoc test selection: the four tests above are the complete committed set.
...
resolution the Lead-1 loss knobs target.
