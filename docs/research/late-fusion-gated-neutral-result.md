...
with a scalar gate (`mkt` = the same model with text masked off). The drag was not a data-structure
issue; it was the gate opening on harmful text. The SupCon contrastive term trains the text rep
to look label-useful on the train fold, and nothing penalised an open gate, so it opened and
overfit text noise, hurting OOS.
