...
The project was originally proposed as a central-bank-sentiment → **FX volatility** predictor, then drifted to SPX equities and rates; the FX target was never actually tested. The dollar index appears only as a side correlation feature (`corr_dxy`), never as a forecast target. FX is therefore **open, not null**, and worth one disciplined shot. The dollar is arguably the most Fed-sensitive asset (the purest expression of relative monetary policy), so if FOMC text helps anywhere, FX volatility is a fair place to look.
...
- **Baseline:** QLIKE-DLq ensemble (5 seeds × 5 folds, 300 epochs) on HAR lags
  `[logRV_t-1, mean-5, mean-22]` — the same machinery that beats HAR on SPX RV.
...
0 / 3 hits. Adding FOMC text (stance + certainty) makes the dollar-vol forecast worse at every
horizon; Bonferroni CIs sit at or below zero. **Verdict: NULL.**

The `r²` daily proxy is noisy (note the high base QLIKE at h1), so the DL ensemble already strains
on the target and the two extra text columns mostly inject noise, most visibly at h1. The direction
is consistent across horizons: text does not help, it hurts. FX, the original and most
Fed-sensitive target, behaves like SPX. Intraday FX (cleaner RV) is not on disk; a clean null at
n≈12.9k days is informative, and a positive result would have been the one to chase with intraday
data, but no positive result is on offer.

**Decision:** null → the original FX target is closed too. Text is null for forecasting on the very
question the project was founded on.
