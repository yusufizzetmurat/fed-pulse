...
So the front of the curve gets its own pre-registered shot, using the strongest single
text feature (policy stance) and an event-reaction framing rather than a stale daily level.
...
- **Hit →** the rates/forward-guidance channel carries real text signal; report it and treat
  it as the project's second (and only text-based) forecasting edge.
- **No hit →** text is fully closed for forecasting across every channel with an economic
  motivation (equity direction, magnitude, volume, volatility, and now rates). Report the null
  and stop. No iteration on feature transform, window, or tenor to manufacture significance.

## Honest ceiling

Even a hit is a low-frequency (~8 meetings/yr), front-end-rates result — a clean research
finding and a risk/positioning signal, not a high-capacity trading edge.
...
The test is well-powered rather than weak: the stance feature varies strongly (std 0.58, full
dovish↔hawkish swing) and there are 110 OOS events. The null is informative. The OLS baseline
itself loses to "predict zero" in 3 of 4 cells — short-end yield reactions are essentially
unforecastable from pre-meeting features, and stance adds nothing.

Caveat: the stance *level* is tested here, not the stance *surprise* against a market
expectation, which the available data does not provide. The announcement-day reaction `r1`
should respond to the surprise component, and it shows no signal in either sign — consistent
with the efficient-information story that by the time the statement text is public, the
reaction is already priced. Per the pre-registered rule, no iteration into a
surprise-construction variant.

Decision (per pre-registration): null → text is fully closed for forecasting across every
economically-motivated channel (equity direction, magnitude, volume, volatility, rates). Text
remains valuable as *description* (stance/certainty/forward-looking labels, semantic diff) —
not as a market predictor. The one forecasting edge in the project stays the QLIKE-DLq RV
ensemble.
