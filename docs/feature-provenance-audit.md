...
| `vix_features` (6-dim VIX term-structure + VRP block) | `loaders._compute_vix_features_for_event` reads the `vix_t_minus_1` / `vix1m_t_minus_1` / `vix3m_t_minus_1` / `vix6m_t_minus_1` / `vix_3m_over_1m_slope` / `vrp_t_minus_1` columns off events.parquet; the columns themselves are populated by `event_dataset_builder._vix_term_structure_features` which reads each ^VIX family close strictly before `event_date` from the cross-asset cache and computes `vrp = vix/100/sqrt(252) - rolling_30d_realized_vol_t_minus_1` against the asset's own close history (also strictly prior). | `T-Δ` (every input dated strictly before `T`) | none | #478 VIX term-structure + VRP. Six scalars: the four constant-maturity ^VIX closes at T-1 (^VIX 1990+, ^VIX1M 2008+, ^VIX3M 2007-12+, ^VIX6M 2008+), the `vix3m / vix1m` slope at T-1 (contango/backwardation signal), and the volatility risk premium at T-1 (implied 30-day vol on a daily scale minus trailing 30-trading-day realised vol). Per-scalar coverage: each ^VIX series degrades to `None` independently before its CBOE listing date; pre-coverage events emit zeros on the affected scalars and the missing flag fires at the event level only when every scalar collapsed. The realised leg of the VRP is `std(log_return(close[t-30..t-1]))` over the supervised asset's own close series, strictly before `event_date` by the `_rolling_realized_vol_t_minus_1` contract. Block is appended past the vote-tally tail by `FeatureVector.as_rich_list` only when the loader populates the slot under `--use-vix-features`. |
| `vix_features_missing` | loader flag on the VIX composer | `T (snapshot)` | none | per-row 0/1 mask. `1.0` when `--use-vix-features` is off (default), the events.parquet predates #478 entirely, or every per-scalar value collapsed to `None` (event sits before ^VIX listing or every fetch failed). The conditional-emit contract on `as_rich_list` keeps the per-bar feature size byte-identical to pre-#478 in those cases. |
| `doc_length` (single scalar) | `loaders.load_walk_forward_split` reads the `token_count` column off events.parquet (whitespace-token count of the row's text body, populated by `event_dataset_builder` at TP build time), computes `log(1 + token_count)`, and broadcasts the result onto every bar of the supervised sequence when `use_doc_length=True`. | `T (snapshot)` | none | #543 doc_length feature. Single observable from the released document itself (a whitespace tokenisation of the text body). No leak surface — the only input is the same text the loader reads for the rest of the rich-feature block. Slot is appended past the VIX tail by `FeatureVector.as_rich_list` only when populated; default off keeps the byte-identical pre-#543 per-bar feature size. Promoted out of the §6.38 confounder ablation cell that produced the largest single dual-head lift (+0.052 macro-F1). |
| `confounder_features` (variable-width control block) | `scripts/run_confounder_ablation._attach_confounder_block` derives the block from per-event metadata read off events.parquet: `event_date` → year one-hot, `event_kind` → meeting-kind one-hot over the six canonical kinds, `token_count` → `log(1 + n)` scalar | `T (snapshot)` | none | #495 confounder ablation. All three control inputs are observable on `T` from the released document (calendar date, document kind, document length). No leak surface. Block width depends on the active cell — empty under `baseline`, year-range-wide under `year_fe`, 6 under `meeting_type_fe`, 1 under `doc_length`, and concatenated under `all_three`. Block is appended past the vote-tally tail by `FeatureVector.as_rich_list` only when the runner writes the slot. |
...
| `forward_realized_vol_1d` | `event_dataset_builder._forward_realized_vol` with `window=1` (closes `[T..T+1]`) | **`T+Δ`, future-derived** | none (target) | #480 multi-horizon auxiliary target. Same strict-forward generator as `forward_realized_vol_10d`; the n=1 degenerate case collapses to `\|log(close[T+1]/close[T])\|` (sample-std ddof=1 is undefined for a single observation). 10d remains canonical for the regime head; this rides on the parquet so downstream heads can be mounted without a rebuild. `None` when the event sits within 1 trading day of the end of the asset price series. |
| `forward_realized_vol_3d` | `event_dataset_builder._forward_realized_vol` with `window=3` (closes `[T..T+3]`) | **`T+Δ`, future-derived** | none (target) | #480 multi-horizon auxiliary target. Same generator and emission contract as `forward_realized_vol_10d`. `None` when the post-event window runs off the end of the price series. |
| `forward_realized_vol_5d` | `event_dataset_builder._forward_realized_vol` with `window=5` (closes `[T..T+5]`) | **`T+Δ`, future-derived** | none (target) | #480 multi-horizon auxiliary target. Same generator and emission contract as `forward_realized_vol_10d`. `None` when the post-event window runs off the end of the price series. |
| `forward_realized_vol_20d` | `event_dataset_builder._forward_realized_vol` with `window=20` (closes `[T..T+20]`) | **`T+Δ`, future-derived** | none (target) | #480 multi-horizon auxiliary target. Same generator and emission contract as `forward_realized_vol_10d`. `None` when the post-event window runs off the end of the price series — note the 20d horizon crosses the next-scheduled-FOMC blackout for some meetings, which is acceptable for an auxiliary target. |
| `forward_realized_vol_30d` | `event_dataset_builder._forward_realized_vol` with `window=30` (closes `[T..T+30]`) | **`T+Δ`, future-derived** | none (target) | #480 multi-horizon auxiliary target. Same generator and emission contract as `forward_realized_vol_10d`. `None` when the post-event window runs off the end of the price series — the 30d horizon spans the next-scheduled-FOMC meeting for most events, which is acceptable for an auxiliary target. |
...
| `target_time_idx` / `target_time_present` | `event_row.axis_time_label` mapped to canonical index (forward looking / not forward looking) | `T (snapshot)` | none (target) | multi-task head training target; the document's forward-looking (time-horizon) label. |
